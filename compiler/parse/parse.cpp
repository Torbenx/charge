...
[[gnu::noinline]] static void emitToken(TokenKind kind, const char* begin, uint32_t data, ParseState& state) {
...
[[nodiscard]] [[gnu::noinline]] static WordAndPosition readWord(const char* position, ParseState& state) {
...
[[gnu::noinline]] static void commitStaticDeclaration(DeclarationKind kind, Word name, const TokenInfo* parseLocation, ParseState& state) {
    bool ret = state.pushStaticScope(kind, name, parseLocation);
    if (ret)
        VERIFY(kind == DeclarationKind::Namespace);
}

[[gnu::noinline]] static void commitHasMemberDeclaration(const TokenInfo* parseLocation, ParseState& state) {
    state.pushHasScope(parseLocation);
}

[[gnu::noinline]] static void commitMemberDeclaration(Word name, const TokenInfo* parseLocation, ParseState& state) {
    bool ret = state.pushMemberScope(name, parseLocation);
    VERIFY(!ret);
}

template<DeclarationKind kind>
static void commitDeclaration(Word name, const TokenInfo* declarationBegin, ParseState& state) {
...
void parseImpl(const char* sourceBufferPosition, ParseState& state, ErrorHandler* errorHandler) {
    ...
    Word word;
    uint32_t tokenData = 0;
    const TokenInfo* declarationBegin = nullptr;
...
after_expression$as_then:
    ...
    case '(': {
        ...
        carriedEmitTokenKind = TokenKind::CallExpr;
        // next first_argument_paren
        goto first_argument_paren$with_emit;
    }
    case ')': {
        ...
        if (scopePosition[0] == ScopeKind::VariableType) {
            ...
            emitToken(TokenKind::AssignStmt, tokBegin, tokenData, state);
            tokenData = 0;
            // emitToken TokenKind::ExpressionStmt
            carriedEmitTokenKind = TokenKind::ExpressionStmt;
            // next after_parameters
    ...
    case '[': {
        ...
        carriedEmitTokenKind = TokenKind::IndexExpr;
    ...
    case '{': {
        ...
        carriedEmitTokenKind = TokenKind::Parameterize;
    ...
    if (isWordFirstCharacter(tokEnd[0])) {
        ...
        LABEL_MAYBE_UNUSED comma_after_expression$keyword_check:
            ...
            if (scopePosition[0] == ScopeKind::VariableType) {
                ...
                tokenData = 0;
                // emitToken TokenKind::ExpressionStmt
                emitToken(TokenKind::ExpressionStmt, tokBegin, tokenData, state);
                tokenData = 0;
                // then parameter
                goto parameter$keyword_check;
            }
...
after_parameters$no_emit:
    ...
    // emitToken TokenKind::EmptyNode
    emitToken(TokenKind::EmptyNode, tokBegin, tokenData, state);
    tokenData = 0;
...
no_declaration$as_then:
    ...
    if (tokEnd[0] == '\0') {
        ...
        emitToken(TokenKind::EOS, tokBegin, tokenData, state);
        tokenData = 0;
